...
//startGameLoop 메서드 - 각 병사별 훈련 버튼(actionButton )이 추가 , 버튼 클릭 시 병사의 만족도와 훈련 점수를 실시간으로 갱신
//병사별 훈련 버튼(actionButton)과 각 병사의 상태를 갱신 - 만족도 -3 훈련점수 +5
...
public class MilitaryLifeSimulation_4 {
    ...
    private static void createShowGUI(JFrame frame) {
        ...
        menu.addSeparator(); //참고 출처: https://blog.naver.com/javaking75/140170842512
    ...
    private static void startGameLoop(JFrame frame) {
        ...
        for (int i = 0; i < soldierNames.length; i++) {
            int index = i; // 각 병사에 대해 고유한 인덱스 저장

            JPanel individualPanel = new JPanel(new BorderLayout());
            JLabel imageLabel = new JLabel();
            ImageIcon icon = new ImageIcon(soldierImages[index]);
            Image scaledImage = icon.getImage().getScaledInstance(150, 150, Image.SCALE_SMOOTH);
            imageLabel.setIcon(new ImageIcon(scaledImage));

            JLabel nameLabel = new JLabel(soldierNames[index], SwingConstants.CENTER);
            nameLabel.setFont(new Font("맑은 고딕", Font.BOLD, 16));

            JLabel statsLabel = new JLabel("만족도: " + satisfactionMap.get(soldierNames[index]) + " | 훈련 점수: " + taskPointsMap.get(soldierNames[index]) + " | 휴가: " + leaveMap.get(soldierNames[index]),
                                           SwingConstants.CENTER);
            statsLabel.setFont(new Font("맑은 고딕", Font.PLAIN, 14));

            JButton actionButton = new JButton(soldierNames[index] + " 훈련");
    ...
    private static void assignTask(String soldier) {
        ...
        if (task != null) {
            ...
            switch (task) {
                ...
                case "근무":
                    satisfaction -= 2;
                    JOptionPane.showMessageDialog(null, soldier + " 근무 할당 완료.", "근무 할당 결과", JOptionPane.INFORMATION_MESSAGE);
                    break;
                case "휴가":
                    if (leave > 0) {
                        leave -= 1;
